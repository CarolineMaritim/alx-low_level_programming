...
void write_abiversion(unsigned char *e_ident);
void write_os_abi(unsigned char *e_ident);
void write_type(unsigned int e_type, unsigned char *e_ident);
void write_entrypoint(unsigned long int e_entry, unsigned char *e_ident);
void close_elf(int elf);
/**
 * elf_checker - Checks if a file is an ELF file.
 * @e_ident: A pointer to content checked.
 */
void elf_checker(unsigned char *e_ident)
{
	int i;
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	write_data(header->e_ident);
	write_version(header->e_ident);
	write_os_abi(header->e_ident);
	write_abiversion(header->e_ident);
	write_type(header->e_type, header->e_ident);
	write_entrypoint(header->e_entry, header->e_ident);
	free(header);
	close_elf(open_fd);
	return (0);
}
